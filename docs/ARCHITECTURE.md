...
The **Omarchy Sec** bridges four core layers:
...
3. **UI Integration Layer (Quickshell):** Native desktop plugin mounted in Omarchy's top status bar (`~/.config/omarchy/plugins/io.github.maxi8594.omarchy-sec/`), providing real-time visual health checks and popout management.
4. **Autonomous AI Responder Layer:** A persistent systemd user daemon (`omarchy-sec-watcher`) monitoring structured alerts in real time, triggering `omarchy-sec-incident` to launch the AI coding agent in a floating Alacritty terminal when high-severity threats are detected.
