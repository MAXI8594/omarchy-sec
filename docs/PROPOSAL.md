...
| [`omarchy-sec-plugin`](https://github.com/MAXI8594/omarchy-sec-plugin) | Quickshell bar widget and popout panel showing sensor status. Separate repository, installed with `omarchy plugin add`. It runs `/usr/bin/omarchy-sec-detect` and refuses every other path, re-checking before each run that it is a `root`-owned, non-symlink, non-world-writable executable; anything else renders as `unknown`. The detector runs inside a named `systemd-run --user --scope` under a `timeout` and a 64 KiB output budget. |
...
A third finding, from the marketplace review of the widget: it validated the detector by path and then executed it, and one of the paths it accepted was `~/.local/bin/omarchy-sec-detect` — a file the user can rewrite between the check and the execution (TOCTOU). The candidate was removed rather than patched; `/usr/bin` is the only path left, for the detector and for the agent binary behind **Call Agent**, each validated separately. The deliberate consequence: installing the CLI from a git checkout no longer feeds the widget, which sits in `unknown` until the package puts the binaries in `/usr/bin`. For a security indicator that is the right answer, and it is a rough edge for anyone running from git.
